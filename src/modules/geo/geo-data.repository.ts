...
export class GeoDataRepository {
  ...
  async findByHotelId(hotelId: string): Promise<GeoData[]> {
    return this.geoDataRepository.find({ where: { hotel: { id: hotelId } } });
  }
  ...
  async findByHotelLanguageAndTitle(hotelId: string, language: TLanguage, title: string): Promise<GeoData | undefined> {
  ...
  async findByHotelAndType(hotelId: string, type: TCategory): Promise<GeoData[]> {
  ...
  async updateGeoDataList(id: string, geoDataList: TGeoData[]): Promise<void> {
