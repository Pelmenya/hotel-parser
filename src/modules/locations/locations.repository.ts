...
export class LocationsRepository {
  ...
  async findRussianLocationsWithoutEnglishTranslation(batch: number): Promise<Locations[]> {
    return this.locationsRepository.createQueryBuilder("location")
      .where("location.language = :ru", { ru: 'ru' })
      .andWhere("location.is_translated_to_en = :isTranslated", { isTranslated: false })
      .take(batch)
      .getMany();
  }
  ...
  async findHotelId(locationId: string): Promise<string | undefined> {
    const location = await this.locationsRepository.createQueryBuilder("location")
